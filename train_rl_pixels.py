...
flags.DEFINE_integer(
    "eval_interval", None, "How often to evaluate. Defaults to not evaluating."
)
flags.DEFINE_integer("eval_episodes", 10, "Number of evaluation episodes.")
flags.DEFINE_integer("batch_size", 64, "Batch size.")
flags.DEFINE_integer(
    "rollout_length", 5, "Number of timesteps to roll out in world model."
)
...
flags.DEFINE_integer(
    "num_behavior_cloning_steps",
    0,
    "Number of behavioral cloning steps before we do offline RL training.",
)
flags.DEFINE_integer(
...
flags.DEFINE_boolean("checkpoint_agent", False, "Whether to checkpoint agent.")
flags.DEFINE_boolean(
...
def process_dataset_to_latents(
    ...
    latent_trajectories = []
    episodes = (
        random.choices(ds._episodes, k=num_episodes)
    ...
    for episode in episodes:
        ...
        episode = jax.tree_util.tree_map(lambda x: jnp.expand_dims(x, axis=1), episode)
...
def latent_rollout(
    ...
    for _ in range(sequence_length):
        ...
        agent._state, actions = agent._act(agent._state, features, 0, eval=False)
    ...
    if reward_penalty_coef > 0.0:
        ...
        pseudo_batch = batch._replace(observations=reconstructed_observations)
        model._state, penalties = model._compute_uncertainty(model._state, pseudo_batch)
        assert batch.rewards.shape == penalties.shape

        batch = batch._replace(rewards=batch.rewards - reward_penalty_coef * penalties)
...
def main(_):
    ...
    representation_dim = (
        rssm_config.gru_hidden_size
        + rssm_config.stoch_discrete_dim * rssm_config.stoch_dim
    )
    ...
    buffer_dir = os.path.join("latent_buffers", FLAGS.env_name, FLAGS.dataset_level)
    ...
    # do agent pretraining via BC on the processed trajectories
    for i in tqdm.tqdm(
        range(FLAGS.num_behavior_cloning_steps),
        smoothing=0.1,
        disable=not FLAGS.tqdm,
        desc="Behavioral cloning",
    ):
        batch = replay_buffer.sample(FLAGS.batch_size, from_real=True)

        agent._state, metrics = agent._update(agent._state, batch, step=i)

        if i % FLAGS.log_interval == 0:
            for k, v in metrics.items():
                wandb.log({f"pretrain/{k}": v}, step=i + checkpoint_num)
        ...
        if FLAGS.eval_interval is not None and i % FLAGS.eval_interval == 0:
            ...
            for k, v in eval_metrics.items():
                wandb.log({f"pretrain-evaluation/{k}": v}, step=i + checkpoint_num)

    # now train the agent in a model-based fashion
    for i in tqdm.tqdm(
        ...
        real_batch = replay_buffer.sample(FLAGS.batch_size, from_real=True)
        mb_batch = replay_buffer.sample(FLAGS.batch_size, from_real=False)
        combined_batch = combine_batches(real_batch, mb_batch)

        # --- update the agent ---

        agent._state, metrics = agent._update(agent._state, combined_batch, step=i)
