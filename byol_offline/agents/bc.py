...
class BC:
    def __init__(self, cfg):
        ...
        if cfg.task not in MUJOCO_ENVS:
            encoder_params = encoder.init(k1, batched_zeros_like(cfg.obs_shape))
            actor_params = actor.init(k2, batched_zeros_like(20000))
        else:
            encoder_params = None
            actor_params = actor.init(k2, batched_zeros_like(cfg.obs_shape))
        ...
        def act(obs: jnp.ndarray, eval_mode: bool):
            ...
            actor_params = self.train_state.params
        ...
        def update(train_state: BCTrainState, transitions: Transition, step: int):
            ...
            loss, (encoder_grads, actor_grads) = loss_grad_fn(train_state.encoder_params, train_state.params, transitions.obs, transitions.actions, key, step)
