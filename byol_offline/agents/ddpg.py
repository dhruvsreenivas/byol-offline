...
class DDPGLearner(ReinforcementLearner):
    ...
    def __init__(
        ...
        representation_dim = (
            ...
            if config.encoder.pixel.dreamer
            else 4096
        )
        ...
        def act(
            ...
            features = encoder.apply(
                state.encoder_params, obs
            )  # don't need batch dim here
        ...
        def bc_update(
            state: DDPGState, batch: Batch, step: int
        ...
        def critic_loss(
            ...
            q_loss = jnp.mean(jnp.square(q1 - target_q)) + jnp.mean(
        ...
        def actor_loss(
            ...
            features = jax.lax.stop_gradient(
                features
            )  # just so we don't have to deal with gradients passing through no matter what
        ...
        def update_critic(
            state: DDPGState, batch: Batch, key: chex.PRNGKey, step: int
        ) -> Tuple[DDPGState, MetricsDict]:
            """Update the critic."""
            ...
            enc_update, new_encoder_opt_state = encoder_optimizer.update(
                encoder_grads, state.encoder_opt_state
            )
        ...
        def update_actor(
            state: DDPGState, batch: Batch, key: jax.random.PRNGKey, step: int
        ) -> Tuple[DDPGState, MetricsDict]:
            """Update the actor."""
        ...
        def update(
            ...
            return state, {**critic_metrics, **actor_metrics}
