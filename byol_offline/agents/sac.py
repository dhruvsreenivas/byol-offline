...
class SACLearner(ReinforcementLearner):
    ...
    def __init__(
        ...
        representation_dim = (
            ...
            if not (
                is_pixel_based(observation_space)
                and config.observation_repr_dim is None
            )
            else 20000
            if not config.encoder.pixel.dreamer
            else 4096
        )
        ...
        log_temperature = jnp.asarray(0.0, dtype=jnp.float32)
        ...
        def act(
            state: SACState, observation: chex.Array, step: int, eval: bool = False
        ...
        def bc_loss(
            ...
            features = encoder.apply(
                encoder_params, batch.observations
            )  # no need to expand batch dim
            distribution = actor.apply(actor_params, features)

            sampled_actions = distribution.sample(seed=key)
            loss = jnp.mean(
                jnp.square(sampled_actions - batch.actions)
            )  # mse loss, exactly like DrQ + BC
            return loss, {"bc_loss": loss}

        def bc_update(
            state: SACState, batch: Batch, step: int
            ...
            (encoder_grads, actor_grads), metrics = grad_fn(
                state.encoder_params, state.actor_params, batch, update_key, step
            )

            # encoder update
            encoder_update, new_encoder_opt_state = encoder_optimizer.update(
                encoder_grads, state.encoder_opt_state
            )
            new_encoder_params = optax.apply_updates(
                state.encoder_params, encoder_update
            )

            # actor update
            actor_update, new_actor_opt_state = actor_optimizer.update(
        ...
        def critic_loss(
            ...
            target_q = jax.lax.stop_gradient(
                batch.rewards + discount * batch.masks * nv
            )
            ...
            tiled_actions = jnp.tile(
                expanded_actions, (cql_samples, 1, 1)
            )  # [cql_samples, B, action_dim]
            tiled_actions = jax.random.uniform(
                cql_key, shape=tiled_actions.shape, dtype=tiled_actions.dtype
            )
            tiled_actions = jnp.concatenate(
                [tiled_actions, expanded_actions], axis=0
            )  # [cql_samples + 1, B, action_dim]

            expanded_features = jnp.expand_dims(features, 0)  # [1, B, feature_dim]
            tiled_features = jnp.tile(
                expanded_features, (cql_samples + 1, 1, 1)
            )  # [cql_samples + 1, B, feature_dim]
            ...
            # compute all losses
            q1_loss = cql_alpha * (q1_penalty.mean() - q1[: B // 2].mean()) + jnp.mean(
                jnp.square(q1 - target_q)
            )
            q2_loss = cql_alpha * (q2_penalty.mean() - q2[: B // 2].mean()) + jnp.mean(
                jnp.square(q2 - target_q)
            )
        ...
        def actor_loss(
            ...
            features = jax.lax.stop_gradient(
                features
            )  # no gradient flow through encoders
        ...
        def update_critic(
            ...
            critic_update, new_critic_opt_state = critic_optimizer.update(
        ...
        def update_temperature(
            ...
            (
                temperature_update,
                new_temperature_opt_state,
            ) = temperature_optimizer.update(grads, state.temperature_opt_state)
            new_log_temperature = optax.apply_updates(
                state.log_temperature, temperature_update
            )
        ...
        def update(
            ...
            # critic update
            state, critic_metrics = update_critic(state, batch, critic_key)

            # actor update
            state, actor_metrics = update_actor(state, batch, actor_key)
            ...
            state = state._replace(
                target_critic_params=new_target_critic_params, rng_key=state_key
            )
