...
class CondAE(hk.Module):
    ...
    def __init__(self, config: ConfigDict):
        ...
        assert (
            len(config.observation_dim) == 1
        ), "This CondAE only supports MuJoCo based envs."
...
class VAE(hk.Module):
    ...
    def __init__(self, config: ConfigDict):
        ...
        assert (
            len(config.observation_dim) == 1
        ), """This VAE only supports mujoco based envs."""
        self._feature_dim = config.feature_dim

        self._encoder = hk.nets.MLP(
            config.hidden_dims, activation=jax.nn.relu, activate_final=False
        )
        self._decoder = hk.nets.MLP(
            [*config.hidden_dims, config.action_dim],
            activation=jax.nn.relu,
    ...
    def __call__(self, s: chex.Array, a: chex.Array) -> DeterVAEOutput:
        ...
        )  # sampling already does reparameterization trick in distrax
...
class AutoEncoderLearner(Learner):
    ...
    def __init__(
        ...
        self._state = AEState(
            ae_params=params, ae_opt_state=opt_state, rng_key=state_key
        )
        ...
        def loss_fn(
            ...
            if config.ae_type == "cond_ae":
                ...
                standard_gaussian = distrax.MultivariateNormalDiag(
                    jnp.zeros((feature_dim,)), jnp.ones((feature_dim,))
                )
                kl = post_dist.kl_divergence(standard_gaussian)
                kl = jnp.mean(kl)
                loss = (
                    rec_loss + beta * kl
                )  # want to minimize KL and reconstruction loss
        ...
        def update(
            ...
            new_state = AEState(
                ae_params=new_params, ae_opt_state=new_opt_state, rng_key=state_key
            )
