...
class MLPDynamicsModel(hk.Module):
    ...
    def __init__(
        self, observation_dim: int, hidden_dims: Sequence[int], act: str = "relu"
    ):
        ...
        self._activation = (
            getattr(jax.nn, act) if hasattr(jax.nn, act) else getattr(jnp, act)
        )

    def __call__(self, obs: chex.Array, action: chex.Array) -> chex.Array:
        obs_action = jnp.concatenate([obs, action], -1)
        x = hk.nets.MLP(
            [*self._hidden_dims, self._observation_dim], activation=self._activation
        )(obs_action)
...
def make_simple_dynamics_network(
    ...
    def forward_fn(s: chex.Array, a: chex.Array):
        model = MLPDynamicsModel(
            observation_dim, config.hidden_dim, act=config.activation
        )
        return model(s, a)
...
class SimpleDynamicsLearner(Learner):
    ...
    def __init__(
        ...
        assert (
            config.model_type == "mlp_dynamics"
        ), "Haven't implemented more complicated models yet. Starting simple :)"
        ...
        optimizer = (
            optax.sgd(config.learning_rate, momentum=0.9, nesterov=True)
        ...
        self._state = SimpleDynamicsState(params=params, opt_state=opt_state)
