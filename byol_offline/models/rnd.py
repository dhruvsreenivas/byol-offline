...
class MLPRNDModel(hk.Module):
    ...
    def __init__(self, config: ConfigDict):
        ...
        self._encoder = hk.nets.MLP(config.encoder_dims, activation=jax.nn.swish)
        self._predictor = RNDPredictor(config.hidden_dim, config.repr_dim)
...
class RNDLearner(Learner):
    ...
    def __init__(
        ...
        def make_initial_state(key: chex.PRNGKey) -> RNDState:
            ...
            state = RNDState(
                params=params, target_params=target_params, opt_state=opt_state
            )
            return state
