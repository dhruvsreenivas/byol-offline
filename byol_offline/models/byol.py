...
class ConvWorldModel(hk.Module):
    ...
    def __init__(self, config: ConfigDict, observation_channels: int):
        ...
        self._rssm = RSSM(
            config.rssm
        )  # uses both closed gru and open gru here, closed for dreamer loss, open for byol loss
        ...
        self._reward_predictor = hk.nets.MLP(
            [*config.reward_hidden_dims, 1], w_init=initializer
        )
    ...
    def _onestep_imagine(
        self, action: chex.Array, state: RSSMState
    ) -> Tuple[RSSMState, ImagineOutput]:
    ...
    def _onestep_observe(
        self, emb: chex.Array, action: chex.Array, state: RSSMState
    ) -> Tuple[RSSMState, ObserveOutput]:
    ...
    def _dreamer_forward(
        ...
        post_img_mean = hk.BatchApply(self._decoder)(
            rssm_output.post_features
        )  # [T, B, H, W, C]
        post_reward_mean = hk.BatchApply(self._reward_predictor)(
            rssm_output.post_features
        )
        ...
        prior_reward_mean = hk.BatchApply(self._reward_predictor)(
            rssm_output.prior_features
        )
        prior_reward_mean = jnp.squeeze(prior_reward_mean)

        # return
        return DreamerOutput(
            post_observation_mean=post_img_mean,
            post_reward_mean=post_reward_mean,
            posts=rssm_output.posts,
            prior_observation_mean=prior_img_mean,
            prior_reward_mean=prior_reward_mean,
            priors=rssm_output.priors,
        )
    ...
    def _byol_forward(
        ...
        pred_latents = jnp.concatenate(
            [latent, latents], axis=0
        )  # [T, B, embed_dim] for both pred_latents and embeds
    ...
    def _process_to_latent(
        ...
        def _posterior_feature_scan_fn(
...
class MLPWorldModel(hk.Module):
    ...
    def __init__(self, config: ConfigDict, observation_dim: int):
        ...
        self._encoder = hk.nets.MLP(
            [*config.hidden_dims, config.repr_dim], activation=jax.nn.swish
        )
        ...
        self._decoder = hk.nets.MLP(
            [*config.hidden_dims, observation_dim], activation=jax.nn.swish
        )
        self._reward_predictor = hk.nets.MLP(
            [*config.reward_hidden_dims, 1], w_init=initializer
        )
    ...
    def _dreamer_forward(
        ...
        post_state_mean = hk.BatchApply(self._decoder)(
            rssm_output.post_features
        )  # [T, B, H, W, C]
        post_reward_mean = hk.BatchApply(self._reward_predictor)(
            rssm_output.post_features
        )  # [T, B, 1]
        post_reward_mean = jnp.squeeze(
            post_reward_mean
        )  # [T, B], so as to be fine with the rewards coming from dataset
        ...
        return DreamerOutput(
            post_observation_mean=post_state_mean,
            post_reward_mean=post_reward_mean,
            posts=rssm_output.posts,
            prior_observation_mean=prior_state_mean,
            prior_reward_mean=prior_reward_mean,
            priors=rssm_output.priors,
        )
...
def make_byol_network(
    ...
    def wm_fn():
        ...
        def observe_fn(
            x: chex.Array, a: chex.Array, s: RSSMState
        ) -> Tuple[RSSMState, ObserveOutput]:
...
class BYOLLearner(Learner):
    ...
    def __init__(
        ...
        rssm_config = getattr(
            config, "pixel" if observations.ndim > 1 else "state"
        ).rssm
        ...
        def _get_latent_dist(
            ...
            stats = jnp.reshape(
                stats, (-1,) + stats.shape[2:]
            )  # [T * B, stoch_dim, stoch_discrete_dim]
        ...
        def _encode(
            state: BYOLState, observations: chex.Array
        ...
        def _decode(
            state: BYOLState, features: chex.Array
        ) -> Tuple[BYOLState, chex.Array]:
            """Decodes features."""
        ...
        def byol_loss_fn_window_size(
            ...
            obs_window = sliding_window(
                obs_seq, starting_idx, window_size
            )  # [T, B, *obs_dims], everything except [T - window_size:] 0s, rolled to front
            action_window = sliding_window(
                action_seq, starting_idx, window_size
            )  # [T, B, action_dim], everything except [T - window_size:] 0s, rolled to front

            pred_latents, _ = byol_forward(
                wm_params, pred_key, obs_window, action_window
            )
            pred_latents = jnp.reshape(
                pred_latents, (-1,) + pred_latents.shape[2:]
            )  # [T * B, embed_dim]

            _, target_latents = byol_forward(
                target_params, target_key, obs_window, action_window
            )  # [T, B, embed_dim]
            target_latents = jnp.reshape(
                target_latents, (-1,) + target_latents.shape[2:]
            )  # [T * B, embed_dim]
            ...
            mses = jnp.square(
                pred_latents - jax.lax.stop_gradient(target_latents)
            )  # [T * B, embed_dim]
        ...
        def byol_loss_fn(
            ...
            def ws_body_fn(ws, curr_state):
                ...
                loss, loss_window = byol_loss_fn_window_size(
                    wm_params, target_params, batch, loss_key, ws
                )
                return curr_loss + loss, curr_loss_window + loss_window, moveon_key

            init_state = (0.0, jnp.zeros((T, B)), key)
            total_loss, total_loss_window, _ = jax.lax.fori_loop(
                1, T + 1, ws_body_fn, init_state
            )
        ...
        def dreamer_loss_fn(
            ...
            observation_log_likelihood = (
                observation_distribution.log_prob(obs_seq) * batch.masks
            )
            reward_log_likelihood = (
                reward_distribution.log_prob(reward_seq) * batch.masks
            )

            reconstruction_loss = (
                -observation_log_likelihood.mean() - reward_log_likelihood.mean()
            )
            ...
            if config.kl_balance == 0.5:
                ...
                kl_loss = (
                    1.0 - config.kl_balance
                ) * left_loss + config.kl_balance * right_loss
        ...
        def update(
            ...
            grads, metrics = grad_fn(
                state.params, state.target_params, batch, update_key
            )
            ...
            new_target_params = optax.incremental_update(
                new_params, state.target_params, ema
            )
            new_state = BYOLState(
                params=new_params,
        ...
        def _initialize_state(
            state: BYOLState, batch_size: int
        ) -> Tuple[BYOLState, RSSMState]:
        ...
        def eval(
        ...
        ) -> Tuple[BYOLState, chex.Array, chex.Array]:
            ...
            obs_seq, action_seq = jnp.expand_dims(obs_seq, 1), jnp.expand_dims(
                action_seq, 1
            )
            ...
            return (
                new_state,
                jax.lax.stop_gradient(post_img_means),
                jax.lax.stop_gradient(prior_img_means),
            )
        ...
        if config.pmap:
            self._update = jax.pmap(
                update, axis_name="devices"
            )  # this is the only function that has any parallel components to it
        else:
