...
def _preprocess(batch: Union[Batch, SequenceBatch]) -> Union[Batch, SequenceBatch]:
    ...
    return jax.tree_util.tree_map(
        lambda x: _process_image(x) if x.dtype == jnp.uint8 else x, batch
    )
...
def _stack_dicts(dataset_dicts: List[DatasetDict], axis: int = 0) -> DatasetDict:
    ...
    for k in dataset_dicts[0].keys():
        if isinstance(dataset_dicts[0][k], Mapping):
            stacked[k] = _stack_dicts(
                [dataset_dict[k] for dataset_dict in dataset_dicts], axis=axis
            )
        else:
            stacked[k] = np.stack(
                [dataset_dict[k] for dataset_dict in dataset_dicts], axis=axis
...
def _dict_to_batch(
    ...
    if "observations" in batch_dict and batch_dict["observations"] is not None:
        if observation_key is not None:
            ...
            if (
                "next_observations" in batch_dict
                and len(batch_dict["next_observations"]) > 0
            ):
            ...
            next_observations = np.reshape(
                next_observations, next_observations.shape[:-2] + (-1,)
            )
        else:
...
class ReplayBuffer(Dataset):
    ...
    def get_iterator(
        self, queue_size: int = 2, sample_args: Mapping = {}
    ) -> Generator[Union[Batch, SequenceBatch], None, None]:
