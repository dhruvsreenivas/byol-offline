...
class LatentReplayBuffer(Dataset):
    ...
    def __init__(
        ...
        real_dataset_dict = dict(
            ...
            next_observations=np.empty((capacity, latent_observation_dim), dtype=np.float32),
    ...
    def insert_mb(self, data_dict: DatasetDict) -> None:
        _insert_recursively(
            self._dataset_dict["mb"], data_dict, self._mb_insert_index
        )
    ...
    def insert_trajectory(self, data_dict: DatasetDict, real: bool = True) -> None:
        ...
        for i in range(traj_len):
            item_dict = jax.tree_util.tree_map(
