...
def main(_):
    ...
    learner = BYOLLearner(config, FLAGS.seed, env.observation_space, env.action_space)
    ...
    for i in tqdm.tqdm(
        ...
        if FLAGS.eval_interval is not None and i % FLAGS.eval_interval == 0:
            learner._state, posterior_images, prior_images = learner._eval(
                learner._state, batch
            )
            ...
            prior_images = [
                Image.fromarray(np.asarray(image), mode="RGB") for image in prior_images
            ]

            for actual, posterior, prior in zip(
                actual_images, posterior_images, prior_images
            ):
                wandb.log(
                    {
